...
// ----- Begin My Code -----
json LteMacVUeMode4::formatted_resource(int ch, double rri){
...
json LteMacVUeMode4::past_max_rri_and_max_ch(){
  ...
  // ----- DEBUG -----
  // _time2ch_rri.clear();
  return this->formatted_resource(max_ch, max_rri);
  // ----- DEBUG -----

  auto itr = _time2ch_rri.begin();
  while (itr != _time2ch_rri.end()) {
    if (1.5 < simTime().dbl() - itr->first) {
...
void LteMacVUeMode4::handleMessage(cMessage *msg)
{
    ...
    else if (incoming == up_[IN])
    {
        if (strcmp(pkt->getName(), "newDataPkt")== 0)
        {
            ...
            // ----- My Code -----
            simtime_t duration = SimTime(lteInfo->getDuration() / 1000.);
            duration = duration - elapsedTime;
            double dur = duration.dbl();
            remainingTime_ = lteInfo->getDuration() - dur;
            // ----- End Modification -----
            ...
            json res = this->past_max_rri_and_max_ch();
            json past_res = res;
            ...
            } else if (res["ch"].get<int>() / res["rri"].get<double>() - res["ch"].get<int>() < required_res){
              // ----- do nothing -----
...
void LteMacVUeMode4::macGenerateSchedulingGrant(double maximumLatency, int priority)
{
    ...
    // ----- Begin My Code -----
    PastschedulingGrant_ = schedulingGrant_;
    // ----- End My Code -----
